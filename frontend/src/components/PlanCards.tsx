      ...
      {plans.map((plan, i) => (
        ...
        >
          ...
          >
            ...
            <h3 className="text-[30px] font-semibold tracking-[-.035em]">{pick(plan.title)}</h3>
            <p className="mt-3 max-w-[26ch] text-[16px] font-light text-fg-2">{pick(plan.note)}</p>
            ...
            <div className="mt-6 border-t border-line pt-[22px]">
              ...
              <span className="mt-[10px] block font-label text-[16px] text-fg-2">{pick(plan.range)}</span>
            </div>
            ...
            <ul>
              {plan.features.map((f) => (
                <li key={f.en} className="flex gap-3 py-2 text-[15.5px] text-fg-2">
                  {pick(f)}
                </li>
              ))}
            </ul>
